...
# Bootstrap путей: app.py лежит в ui/, а соседние модули (notebooks, дизайн,
# cases, …) — в core/ и в корне репо. Streamlit добавляет в sys.path только
# папку самого app.py, поэтому достраиваем sys.path сами, не полагаясь на
# sitecustomize.py.
_РЕПО = Path(__file__).resolve().parent.parent
for _подпапка in ("", "core", "pipeline", "scripts"):
...
load_dotenv()
# APP_DIR указывает на корень репо: картинки лежат в extracted_images/<hash>/
# относительно корня, а сам app.py теперь живёт в ui/.
...
7. Ставь маркер [N] сразу после конкретного утверждения, формулы, числа, термина или цитаты, где N — номер фрагмента из CONTEXT. НЕ ставь пачки маркеров вида [1][2][3][4][5] после общей фразы. На одно утверждение используй 1 маркер, максимум 2 маркера, если два фрагмента действительно независимо подтверждают один и тот же факт. Если несколько фрагментов говорят одно и то же — выбери самый точный, а не перечисляй все. НЕ ставь маркер на общие фразы-связки типа «используются различные методы», «рассмотрим подробнее», «таким образом», «это применяется для». Без маркеров на ключевых фактах ответ считается неполным, но лишние маркеры хуже, чем отсутствие маркера на общей фразе.
...
9. Если в CONTEXT есть химическая формула, реакционное уравнение или математическое выражение по вопросу — ОБЯЗАТЕЛЬНО вынеси его в ответ. Формулы и реакции, даже если в CONTEXT они записаны обычным текстом (`HCHO(aq) + H2O(l) ⇌ CH2(OH)2(aq)`), переписывай в корректный LaTeX. Главные формулы оборачивай в БЛОЧНЫЕ двойные доллары $$...$$ — каждая на отдельной строке с пустыми строками до и после. Inline-доллары $...$ используй ТОЛЬКО для одиночных коротких символов внутри текста. Используй стандартный LaTeX-синтаксис: \\rightleftharpoons для обратимых реакций, \\rightarrow для направленных реакций, нижние индексы через _{...}, степени через ^{...}, \\sigma вместо σ, \\sum_{i=1}^{n} вместо ∑, \\cdot вместо *, \\frac{a}{b} для дробей. Не используй Unicode-математику и псевдо-LaTeX типа h_v^(t+1).
10. ЗАПРЕЩЕНО приводить формулы, уравнения и численные константы, которых НЕТ в CONTEXT — даже если ты эту формулу знаешь из общих знаний. Разрешено только аккуратно переписать формулу из CONTEXT в LaTeX, не меняя смысл и вещества. Если формулы в CONTEXT нет, опиши процесс словами или скажи: «Формула для этого в найденных фрагментах не приведена». Каждая формула в ответе должна быть подкреплена маркером [N] из CONTEXT, где она реально присутствует.
...
13. ЗАПРЕЩЕНО ВЫВОДИТЬ СЛУЖЕБНУЮ ШАПКУ ФРАГМЕНТА. В CONTEXT каждый фрагмент начинается с метки `[N]` — это исключительно служебная разметка для маркеров. НЕ копируй в ответ строки вида «Источник: корпус. Документ: foo.pdf, стр. 15», «Документ: …, стр. …», «Источник: мои документы», «[N] Источник:…». Не перечисляй фрагменты по очереди со своими заголовками. В ответ идут только осмысленные утверждения, переписанные своими словами на русском, с маркерами [N] в нужных местах.
14. ОТВЕТ ДОЛЖЕН БЫТЬ СВЯЗНЫМ ТЕКСТОМ, А НЕ ПЕРЕСКАЗОМ ФРАГМЕНТОВ. Структурируй мысль вокруг вопроса: 1) суть/определение, 2) методы или подходы из CONTEXT, 3) конкретные данные/формулы/числа, 4) (опц.) ограничения или примеры. Не пиши «Фрагмент 1 говорит…», «В фрагменте 2…» — это служебная мысль. Маркеры [N] ставь по факту, не как ссылку «см. фрагмент N». Один документ — одна-две мысли максимум; если в выдаче есть 2-3 документа, обязательно опирайся на разные документы для разных аспектов ответа, а не пересказывай один документ от и до.
15. SMILES-СТРОКИ — структуры молекул. Это ОПЦИОНАЛЬНАЯ фича: используй ТОЛЬКО если SMILES реально присутствует в CONTEXT и однозначно относится к ответу. Если SMILES в CONTEXT нет — НЕ ВЫДУМЫВАЙ. Не пытайся «полезно» сгенерировать SMILES для каждого упомянутого вещества — это сделает ответ хуже. Когда SMILES уместен и взят из CONTEXT — оборачивай его в блок:

```smiles
CC(=O)Oc1ccccc1C(=O)O
```

UI распознаёт такие блоки и рисует структурную формулу. Несколько SMILES — несколько блоков. Маркер [N] ставь сразу после блока. Не путай SMILES с обычной формулой Hill (`C9H8O4`, `Cl2`, `H2SO4` — это формулы, для них блок smiles НЕ нужен). Если в CONTEXT упоминается простое вещество (хлор Cl₂, азот N₂, вода H₂O и т.п.), то химическая формула выводится обычным текстом в LaTeX как описано в правиле 9, без блока smiles.

16. КОГДА В CONTEXT НЕТ ОТВЕТА, НО ВОПРОС ХИМИЧЕСКИЙ — есть отдельный режим, активируемый системой автоматически. Если в user-сообщении после CONTEXT есть пометка `MODE: fallback_general_knowledge`, это значит, что найденные фрагменты не содержат ответа, но вопрос находится в области химии или смежной науки и ты можешь ответить из общих знаний. В этом режиме:
  — НЕ говори «В базе нет данных».
  — Ответь по сути вопроса своими общими знаниями школьного и вузовского уровня (формулы, классы веществ, типичные свойства, базовые реакции).
  — В конце ответа честно добавь курсивом: *Это общеизвестное знание, в найденных фрагментах конкретного ответа не было.*
  — Маркеры [N] в этом режиме НЕ ставь — нет фрагментов, которые их подтверждают.
  — Не выдумывай специфических чисел (точные температуры плавления редких веществ, точные кинетические константы) — только то, в чём ты уверен.

Если пометки `MODE: fallback_general_knowledge` НЕТ — действует обычный протокол (правила 2-15)."""
...
def _ключ_документа(точка):
    payload = getattr(точка, "payload", {}) or {}
    return payload.get("doc_id") or payload.get("document") or str(getattr(точка, "id", ""))


def _диверсифицировать_документы(
    кандидаты,
    количество,
    *,
    max_per_document=2,
    ...
    """Мягкий MMR-lite по документам с эскалацией лимита.

    Цель: дать ответу диверсифицированный набор источников. Один документ
    почти никогда не должен давать половину фрагментов — так LLM скатывается
    в пересказ одной статьи (см. жалобу пользователя «один документ — все
    10 совпадений»).

    Алгоритм трёхуровневый:
      1. Жёсткий проход: max=1 фрагмент на документ среди релевантных
         (score >= лучший × min_score_ratio).
      2. Если набралось < количество — мягкий проход: max=`max_per_document`,
         тоже только среди релевантных. Добавляет 2-й фрагмент из тех же
         документов, что уже попали.
      3. Если всё ещё не хватает — добор без порога score и без лимита
         per-document. Это страховка для узких выдач (5 кандидатов из
         одного PDF — лучше показать их, чем пустой ответ).

    Раньше был один проход `max_per_document=2` без эскалации — для широких
    выдач это давало 5 фрагментов из 3 документов вместо 5 из 5.
    """
    if len(кандидаты) <= количество:
        return кандидаты[:количество]

    def _id_точки(т):
        return str(getattr(т, "id", id(т)))

    лучший_score = max(float(getattr(т, "score", 0.0) or 0.0) for т in кандидаты)
    порог = лучший_score * min_score_ratio if лучший_score > 0 else None

    выбранные: list = []
    выбранные_id: set = set()
    по_документам: dict = {}

    def _взять(лимит_на_документ: int, *, требовать_порог: bool) -> None:
        for т in кандидаты:
            if len(выбранные) >= количество:
                return
            if _id_точки(т) in выбранные_id:
                continue
            if требовать_порог and порог is not None \
                    and float(getattr(т, "score", 0.0) or 0.0) < порог:
                continue
            док = _ключ_документа(т)
            if по_документам.get(док, 0) >= лимит_на_документ:
                continue
            выбранные.append(т)
            выбранные_id.add(_id_точки(т))
            по_документам[док] = по_документам.get(док, 0) + 1

    # Шаг 1 — максимум разнообразия (1 фрагмент на документ, релевантные)
    _взять(лимит_на_документ=1, требовать_порог=True)
    # Шаг 2 — эскалация на 2 на документ, если мало (расширяем глубину)
    if len(выбранные) < количество:
        _взять(лимит_на_документ=max(max_per_document, 2), требовать_порог=True)
    # Шаг 3 — добор без ограничений (узкая выдача, спасаем то что есть)
    if len(выбранные) < количество:
        _взять(лимит_на_документ=10**9, требовать_порог=False)

    return выбранные[:количество]


def _расширить_короткий_запрос(вопрос: str) -> str:
    """Помогает retrieval'у на коротких химических вопросах.

    Кейс из жалобы пользователя: «формула хлора» (3 слова) → e5-base
    кодирует слабо, в найденных топ-5 нет ни одной статьи про Cl₂. На
    запрос «формула хлора Cl2 chlorine molecular formula chemical» поиск
    отрабатывает гораздо лучше, потому что:
      — добавляет английские термины (корпус смешанный ru+en);
      — добавляет химическую формулу как ключевое слово для BM25-поиска.

    Расширение применяется ТОЛЬКО для коротких запросов (≤4 слов): на
    длинных оно бы зашумило выдачу. Сам исходный вопрос всегда идёт первым.
    """
    очищенный = (вопрос or "").strip()
    if not очищенный:
        return очищенный
    слова = очищенный.split()
    if len(слова) > 4:
        return очищенный  # длинный запрос — не трогаем

    # Простой словарь синонимов для частых химических вопросов.
    # Без этого retrieval на коротких вопросах часто промахивается.
    синонимы = {
        "формула": "chemical formula molecular formula structure",
        "хлора": "chlorine Cl2 Cl",
        "хлор": "chlorine Cl2 Cl",
        "брома": "bromine Br2 Br",
        "бром": "bromine Br2 Br",
        "йода": "iodine I2 I",
        "йод": "iodine I2 I",
        "фтора": "fluorine F2 F",
        "фтор": "fluorine F2 F",
        "кислорода": "oxygen O2 O",
        "кислород": "oxygen O2 O",
        "азота": "nitrogen N2 N",
        "азот": "nitrogen N2 N",
        "водорода": "hydrogen H2 H",
        "водород": "hydrogen H2 H",
        "воды": "water H2O",
        "вода": "water H2O",
        "серы": "sulfur S sulfide",
        "сера": "sulfur S sulfide",
        "натрия": "sodium Na",
        "натрий": "sodium Na",
        "калия": "potassium K",
        "калий": "potassium K",
        ...
        "метан": "methane CH4",
        "этан": "ethane C2H6",
        "этанол": "ethanol C2H5OH",
        "метанол": "methanol CH3OH",
        "бензол": "benzene C6H6",
        "молекула": "molecule molecular",
        "молекулы": "molecule molecular",
        "реакции": "reaction",
        "реакция": "reaction",
        "выхода": "yield product",
        "выход": "yield product",
        "катализатор": "catalyst catalysis",
        "катализатора": "catalyst catalysis",
        "токсичности": "toxicity LD50",
        "токсичность": "toxicity LD50",
        "растворимости": "solubility logP",
        "растворимость": "solubility logP",
        "температура": "temperature melting boiling",
        "температуры": "temperature melting boiling",
    }
    добавки: list[str] = []
    видели: set[str] = set()
    for слово in слова:
        ключ = слово.lower().strip(".,?!;:()\"'«»")
        доп = синонимы.get(ключ)
        if доп and доп not in видели:
            видели.add(доп)
            добавки.append(доп)
    if not добавки:
        return очищенный
    # Сохраняем оригинал в начале — он влияет на embedding больше, чем хвост.
    return очищенный + " " + " ".join(добавки)
...
def найти_похожие(
    ...
    *,
    ...
    # C18 fix: на коротких запросах вроде «формула хлора» retrieval часто
    # возвращает мусор. Расширяем короткие запросы химическими синонимами
    # на английском (корпус смешанный) и добавляем формулы как ключи
    # для BM25. Длинные запросы (>4 слов) не трогаем.
    вопрос_расширенный = _расширить_короткий_запрос(вопрос)
    вектор = модель.encode("query: " + вопрос_расширенный, normalize_embeddings=True).tolist()

    if новая_схема:
        # C2 fix: ранее тут был выбранный_кейс=None с комментарием «case в
        # новой схеме менее приоритетен», и UI-селектор «Фильтр по кейсу»
        # становился мёртвой фичей — пользователь выбирал, выдача не
        # менялась. Поле `case` пишется в payload каждого чанка
        # (ingest_v2.py: "case": кейс) — теперь его реально применяем.
        фильтр = _построить_фильтр(
            выбранный_кейс=выбранный_кейс,
    ...
    множитель = 10 if использовать_reranker else 8
    лимит_кандидатов = min(max(количество * множитель, 40), 100)
    ...
    if гибрид:
        ...
        # Sparse тоже на расширенном запросе — формулы Cl2/H2O это сильные
        # ключевые слова BM25, на коротких запросах без них находит мусор.
        sparse_idx, sparse_val = построить_sparse_один(вопрос_расширенный)
    ...
    if использовать_reranker and кандидаты:
        try:
            ...
            упорядоченные = переранжировать(вопрос, тексты, top_k=None)
    ...
    return _диверсифицировать_документы(кандидаты, количество)
...
def _ключ_deepseek():
    ключ = os.getenv("DEEPSEEK_API_KEY", "").strip()
    return ключ or None
...
def _llm_доступна(llm_id):
    ...
    if provider == "groq":
        return bool(_ключи_groq())
    if provider == "deepseek":
        return bool(_ключ_deepseek())
    return False


def _llm_label(llm_id):
    данные = LLM_МОДЕЛИ.get(llm_id) or LLM_МОДЕЛИ["groq:llama-3.3-70b-versatile"]
    label = данные["label"]
    return label if _llm_доступна(llm_id) else f"{label} (нет ключа)"


def _llm_по_умолчанию():
    for llm_id in ("deepseek:deepseek-v4-flash", "groq:llama-3.3-70b-versatile", "groq:llama-3.1-8b-instant"):
        if _llm_доступна(llm_id):
            return llm_id
    return "groq:llama-3.3-70b-versatile"


def _это_rate_limit(ошибка):
    """Распознаёт все варианты переполнения квот Groq:
      - 429 Too Many Requests (RPM/RPD limit);
      - 413 Request Too Large (TPM, когда CONTEXT > tokens-per-minute cap);
      - тестовые маркеры в сообщении.

    Когда такое ловим — пробуем следующий ключ или резервную модель,
    а не raise — иначе пользователь получает голую APIError."""
    текст = str(ошибка).lower()
    маркеры = (
        "429", "rate_limit", "rate limit", "tokens per day",
        "tokens per minute", "tpm", "request too large",
        "413",
    )
    return any(м in текст for м in маркеры)
...
def вызвать_deepseek(параметры_запроса):
    ключ = _ключ_deepseek()
    if not ключ:
    ...
    payload = dict(параметры_запроса)
    ...
    data = response.json()
    choices = [
        SimpleNamespace(message=SimpleNamespace(content=(choice.get("message") or {}).get("content", "")))
        for choice in data.get("choices", [])
    ]
    return SimpleNamespace(choices=choices)


def вызвать_llm(параметры_запроса, llm_id=None):
    llm_id = llm_id or _llm_по_умолчанию()
    данные = LLM_МОДЕЛИ.get(llm_id)
    if not данные:
        raise RuntimeError(f"Неизвестная модель: {llm_id}")
    параметры = dict(параметры_запроса, model=данные["model"])
    if данные["provider"] == "groq":
        # Fallback-логика:
        # - на 70B (мощная) → пробуем 8B (быстрее, иногда отдаёт когда 70B
        #   упёрся в RPM по аккаунту);
        # - на 8B (слабая) → НЕ пробуем 8B повторно, а пробуем 70B —
        #   у 70B на free tier выше TPM (12 000 vs 6 000 у 8B), поэтому
        #   именно она спасает от 413 «request too large» на длинных
        #   CONTEXT'ах из RAG-фрагментов.
        if данные["model"] == "llama-3.1-8b-instant":
            резерв = "llama-3.3-70b-versatile"
        else:
            резерв = "llama-3.1-8b-instant"
        return вызвать_groq(параметры, резервная_модель=резерв)
    if данные["provider"] == "deepseek":
        return вызвать_deepseek(параметры)
    raise RuntimeError(f"Неизвестный провайдер модели: {данные['provider']}")
...
def _собрать_картинки_фрагмента_для_промпта(payload: dict) -> list[tuple[int, str]]:
    ...
    пригодными для вставки в промпт.

    Без caption картинку не передаём модели — чтобы она не ставила [img:N.M]
    наугад на изображения, смысл которых ей неизвестен.
    Для слайдов с fallback-caption вида «Слайд: …» используем укороченную версию,
    ...
    for индекс, карт in enumerate(картинки, 1):
        ...
        if not caption:
            continue
        # Слайдовый fallback «Слайд: …» полезен для поиска,
        # но слишком длинный для промпта — укорачиваем агрессивнее
        if caption.startswith("Слайд: ") and len(caption) > 150:
            caption = caption[:147].rstrip() + "…"
        elif len(caption) > 400:
            caption = caption[:397].rstrip() + "…"
        результат.append((индекс, caption))
    return результат


def получить_ответ_от_groq(вопрос, фрагменты, *, llm_id=None):
    llm_id = llm_id or _llm_по_умолчанию()
    if not _llm_доступна(llm_id):
        данные = LLM_МОДЕЛИ.get(llm_id) or {}
        provider = данные.get("provider") or "LLM"
        env_name = "DEEPSEEK_API_KEY" if provider == "deepseek" else "GROQ_API_KEY"
        return f"Ошибка: {env_name} не задан в файле .env"

    контекст = ""
    for i, фр in enumerate(фрагменты, 1):
        payload = фр if isinstance(фр, dict) else фр.payload
        # Метку фрагмента делаем минимальной: только `[N]` + отрывок текста.
        # Раньше тут было «[N] Источник: корпус. Документ: <name>, стр. N» —
        # модели вытаскивали эти служебные слова в ответ как «Источник:
        # корпус. Документ: …» (см. C11 на скрине), потому что более слабые
        # LLM просто пересказывали каждый фрагмент со заголовком CONTEXT.
        # Имя документа и страница в ответе не нужны — UI добавляет блок
        # «Источники» сам из метаданных фрагментов.
        контекст += f"[{i}]\n"
    ...
    данные_llm = LLM_МОДЕЛИ.get(llm_id) or {}
    доп_инструкция = ""
    if данные_llm.get("provider") == "deepseek":
        # C12 fix: DeepSeek даже на хорошие фрагменты часто отвечал «В базе
        # нет данных», когда LLaMA-70B на тех же фрагментах писал нормальный
        # развёрнутый ответ. Старая инструкция была мягкой («не отвечай так
        # из-за недословного совпадения») — DeepSeek её игнорировал. Сейчас
        # добавляем жёсткую процедуру: сначала перечислить все факты по
        # теме, потом ответить. Отказ — только если перечень пустой.
        доп_инструкция = (
            "\n\nКРИТИЧНО ДЛЯ DEEPSEEK. CONTEXT — это ТОП-K фрагментов, "
            "уже отобранных гибридным поиском (dense + BM25 + reranker) "
            "под этот вопрос. Это значит, что они РЕЛЕВАНТНЫ. "
            "Поисковая система не вернула бы эти фрагменты, если бы они "
            "не имели смысловой связи с вопросом.\n\n"
            "ЖЁСТКИЙ ПРОТОКОЛ ОТВЕТА:\n"
            "Шаг 1. Молча, для себя, выпиши все термины, методы, формулы, "
            "числа, определения, классы соединений, химические реакции, "
            "которые есть в CONTEXT и хоть как-то относятся к теме вопроса.\n"
            "Шаг 2. Если этот перечень содержит ХОТЯ БЫ ОДИН пункт по теме — "
            "ОБЯЗАТЕЛЬНО дай развёрнутый ответ на основе этого перечня. "
            "Не отвечай 'В базе нет данных'.\n"
            "Шаг 3. Отказ 'В базе нет данных' допустим ТОЛЬКО если перечень "
            "из шага 1 действительно пустой — то есть все фрагменты говорят "
            "о принципиально другой области (например, вопрос про катализ, "
            "а в CONTEXT только статьи по астрофизике без единого химического "
            "термина).\n"
            "Если вопрос — про химию или связанную область, и в CONTEXT "
            "встречаются химические термины, формулы, реакции, классы веществ, "
            "методы анализа — это уже основание для содержательного ответа."
        )

    def _позвать_llm(с_fallback: bool):
        mode_tag = "\n\nMODE: fallback_general_knowledge" if с_fallback else ""
        return вызвать_llm({
            "messages": [
                {"role": "system", "content": системный_промпт + доп_инструкция},
    ...
    # C22 v2: ранее активация fallback-режима шла на основе словарного
    # совпадения вопроса с фрагментами. Это был плохой эвристик: на «GNN
    # применения» (короткие слова <4 букв игнорируются) триггер срабатывал,
    # хотя retrieval нашёл нормальные фрагменты — и модель ставила плашку
    # «общеизвестное знание» поверх абсолютно цитированного ответа.
    #
    # Новый подход — реактивный: сначала пробуем обычный режим. Только если
    # модель РЕАЛЬНО отказалась («В базе нет данных» и ни одного маркера) —
    # делаем второй заход с MODE: fallback_general_knowledge. Это +1 LLM-
    # запрос только в редком случае реального отказа, в обычной работе
    # запросов столько же сколько было раньше.
    ответ = _позвать_llm(с_fallback=False)
    черновой_текст = (ответ.choices[0].message.content or "").strip()
    отказ_без_цитат = (
        ("в базе нет данных" in черновой_текст.lower()
         or "не приведена" in черновой_текст.lower()
         or len(черновой_текст) < 80)
        and not re.search(r"\[\d+\]", черновой_текст)
    )
    if отказ_без_цитат:
        try:
            ответ = _позвать_llm(с_fallback=True)
        except Exception as e:
            print(f"[fallback] второй проход упал: {e}", flush=True)
    текст = ответ.choices[0].message.content
    текст = отрезать_источники(текст)
    текст = _убрать_маркеры_вне_контекста(текст, len(фрагменты))
    ...
    # C22 v2 пост-фильтр: если в финальном ответе есть маркеры цитат [N],
    # значит модель реально опиралась на CONTEXT — плашка «это общеизвестное
    # знание, в найденных фрагментах конкретного ответа не было» становится
    # ложной (модель ошибочно поставила её рефлекторно). Срезаем.
    if re.search(r"\[\d+\]", текст):
        текст = _срезать_плашку_общих_знаний(текст)
    return текст


def получить_углублённый_ответ(вопрос, фрагменты, *, llm_id=None):
    """Двухпроходный ответ. Сначала черновик, потом расширение / кросс-
    проверка через ВТОРУЮ модель (если доступна) или ту же.

    Идея: первый ответ часто упускает половину фактов из CONTEXT, особенно
    при холодной модели. Второй проход видит и фрагменты, и черновик —
    и работает уже как «дополнить, не повторяя».

    Если в системе доступны и Groq и DeepSeek — кросс-валидация: первый
    проход на одной, второй на другой. Это снижает кейс «одна модель
    систематически отказывается».

    Возвращает dict вида:
      {"финал": "...", "черновик": "...", "llm1": "...", "llm2": "..."}
    Если что-то пошло не так — финал может равняться черновику; в любом
    случае ключ "финал" всегда есть и содержит то, что должно показаться
    как основной ответ.
    """
    llm_id = llm_id or _llm_по_умолчанию()
    данные = LLM_МОДЕЛИ.get(llm_id) or {}
    провайдер = данные.get("provider")

    # Выбор второй модели:
    # - Если первая Groq и есть DeepSeek → второй проход DeepSeek.
    # - Если первая DeepSeek и есть Groq → второй проход Groq llama-70b.
    # - Иначе → та же модель, но с другим промптом «дополни, не повторяй».
    если_groq = "groq:llama-3.3-70b-versatile"
    если_deepseek_pro = "deepseek:deepseek-v4-pro"
    если_deepseek_flash = "deepseek:deepseek-v4-flash"

    второй_id = llm_id  # дефолт — та же модель
    if провайдер == "groq" and _llm_доступна(если_deepseek_flash):
        второй_id = если_deepseek_flash
    elif провайдер == "deepseek" and _llm_доступна(если_groq):
        второй_id = если_groq

    # Проход 1: обычный ответ
    черновик = получить_ответ_от_groq(вопрос, фрагменты, llm_id=llm_id)
    ...
    # Если первый проход — это отказ или ошибка ключа, не делаем второй,
    # просто возвращаем как есть.
    отказ_маркеры = ("В базе нет данных", "Ошибка:")
    if any(м in (черновик or "") for м in отказ_маркеры) and not re.search(r"\[\d+\]", черновик or ""):
        return результат

    # Проход 2: расширение
    if not _llm_доступна(второй_id):
        второй_id = llm_id  # fallback на ту же
        результат["llm2"] = второй_id

    контекст_фрагментов = ""
    for i, фр in enumerate(фрагменты, 1):
        payload = фр if isinstance(фр, dict) else фр.payload
        контекст_фрагментов += f"[{i}]\n{payload['text']}\n"
        картинки_с_подписью = _собрать_картинки_фрагмента_для_промпта(payload)
        if картинки_с_подписью:
            контекст_фрагментов += "Картинки:\n"
            for M, caption in картинки_с_подписью:
                контекст_фрагментов += f"  [img:{i}.{M}] {caption}\n"
        контекст_фрагментов += "\n"

    промпт_расширения = (
        системный_промпт +
        "\n\nЗАДАЧА ВТОРОГО ПРОХОДА: ниже приведён ЧЕРНОВИК ответа от другой "
        "модели. Твоя задача — дополнить и улучшить его, а не переписать с нуля. "
        "Сохрани все факты черновика, добавь:\n"
        " — пропущенные методы, формулы, химические реакции (если они в CONTEXT);\n"
        " — пропущенные классы соединений, растворители, катализаторы;\n"
        " — пропущенные количественные данные (числа, диапазоны);\n"
        " — структурную глубину (если черновик из 1 абзаца — разверни в 3-4 "
        "связных раздела со своими маркерами [N]).\n"
        "Не повторяй одно и то же двумя способами. Финал — единый связный "
        "ответ, как будто его сразу написал один человек. НЕ упоминай "
        "слово 'черновик' и не комментируй процесс."
    )

    данные2 = LLM_МОДЕЛИ.get(второй_id) or {}
    if данные2.get("provider") == "deepseek":
        промпт_расширения += (
            "\n\nКРИТИЧНО: даже если в черновике написано 'В базе нет данных', "
            "перечитай CONTEXT. Если хоть один факт по теме там есть — игнорируй "
            "отказ черновика и пиши развёрнутый ответ с нуля по фактам из CONTEXT."
        )
    ...
    user_msg = (
        ...
        f"ВОПРОС:\n{вопрос}\n\n"
        f"ЧЕРНОВИК ОТВЕТА:\n{черновик}\n\n"
        f"Выдай улучшенный ответ."
    )
    try:
        ответ2 = вызвать_llm({
            "messages": [
                {"role": "system", "content": промпт_расширения},
        ...
        print(f"[углублённый] второй проход упал: {e}", flush=True)
        return результат

    финал = ответ2.choices[0].message.content
    финал = отрезать_источники(финал)
    финал = _убрать_маркеры_вне_контекста(финал, len(фрагменты))
    финал = убрать_неверные_маркеры(финал, фрагменты)
    финал = _убрать_маркеры_вне_контекста(финал, len(фрагменты))
    финал = _вырезать_pdf_мусор_из_ответа(финал)
    финал = _ограничить_количество_картинок(финал)
    # Если второй проход вышел КОРОЧЕ черновика, скорее всего он сломался.
    # Возвращаем черновик как страховку.
    if len(финал.strip()) < int(len(черновик.strip()) * 0.7):
        return результат
    результат["финал"] = финал
    return результат
...
def сериализовать_фрагменты(точки):
    ...
    # Предварительно кодируем тексты фрагментов ОДИН раз -
    # чтобы убрать_неверные_маркеры не перекодировала их заново.
    if фрагменты:
        try:
            модель = загрузить_модель()
            тексты = [фр["text"] for фр in фрагменты]
            emb_ф = модель.encode(
                ["passage: " + t for t in тексты], normalize_embeddings=True
            )
            for i, фр in enumerate(фрагменты):
                фр["_embed"] = emb_ф[i].tolist()
        except Exception:
            pass
    return фрагменты
...
# C15: блок ```smiles\n<SMILES>\n``` в ответе LLM. UI вырезает его и
# рендерит молекулу через клиентский smiles-drawer (~50 KB JS, без зависимостей
# на rdkit). Несколько блоков в одном ответе допустимы.
_SMILES_БЛОК = re.compile(
    r"```smiles\s*\n([^\n][^`]*?)\n?```",
    re.IGNORECASE,
)
# Простая валидация SMILES: разрешены буквы атомов, цифры, скобки, кратные
# связи, стерео, заряды. Длина 1-300. Это не строгий парсер — просто фильтр
# от очевидного мусора, который мог попасть в код-блок (URL, фраза и т.п.).
_SMILES_ВАЛИД = re.compile(r"^[A-Za-z0-9@+\-\[\]()=#$:/\\\.\*%]+$")
...
# Плашка fallback-режима, которую модели рекомендуется ставить в правиле 16.
# Регэксп ловит её разные формы: с курсивом *...*, с прописной/строчной,
# с разными формулировками («не было», «отсутствует», и т.п.). Удаляется
# только если в ответе есть реальные маркеры цитат [N] — иначе плашка
# уместна и остаётся.
_ПЛАШКА_ОБЩИХ_ЗНАНИЙ = re.compile(
    r"\n*\*?_?\s*"
    r"Это(?:\s+общеизвестн\w+\s+знани[ея]|\s+ответ\s+из\s+общих\s+знани[йй])"
    r"[^*\n]*?(?:фрагмент\w+|корпус\w*|базе)[^*\n]*?\.?"
    r"\s*_?\*?\n*",
    re.IGNORECASE,
)


def _срезать_плашку_общих_знаний(текст: str) -> str:
    """Удаляет плашку «Это общеизвестное знание, в найденных фрагментах
    конкретного ответа не было» из ответа. Применяется только когда в
    ответе есть маркеры цитат [N] — значит модель РЕАЛЬНО опиралась на
    CONTEXT, и плашка ложная (модель поставила её рефлекторно)."""
    очищенный = _ПЛАШКА_ОБЩИХ_ЗНАНИЙ.sub("\n\n", текст)
    # Свернём двойные пустые строки до одной двойной
    очищенный = re.sub(r"\n{3,}", "\n\n", очищенный).strip()
    return очищенный


def _убрать_маркеры_вне_контекста(ответ: str, всего_фрагментов: int) -> str:
    """Удаляет citation-маркеры, которых нет в текущем CONTEXT."""
    if not ответ or "[" not in ответ:
        return ответ

    def _замена(m: re.Match) -> str:
        номер = int(m.group(1))
        return m.group(0) if 1 <= номер <= всего_фрагментов else ""

    очищенный = re.sub(r"(?<!img:)\[(\d+)\]", _замена, ответ)
    очищенный = re.sub(r" {2,}", " ", очищенный)
    очищенный = re.sub(r"\s+([.,;:])", r"\1", очищенный)
    return очищенный


_МИН_ПЛОЩАДЬ_КАРТИНКИ = 280 * 280  # ~78 400 px²; логотипы/иконки обычно меньше
...
def _картинка_содержательная(путь: Path) -> bool:
    """Отсекает логотипы/иконки: площадь < 280² px² или очень вытянутые (>8:1).
    Проверяем площадь, а не min-сторону — горизонтальные схемы-деревья
    могут быть широкими и невысокими, но всё равно содержательными."""
    размер = _размер_картинки(путь)
    if размер is None:
        return True  # если не смогли прочитать — не режем
    w, h = размер
    if w * h < _МИН_ПЛОЩАДЬ_КАРТИНКИ:
        return False
...
def _собрать_доступные_картинки(картинки, текущая_страница, *, фильтр_размера: bool = True):
    """Возвращает список (путь, страница) для отображения."""
...
def _собрать_доступные_картинки_с_caption(картинки, текущая_страница, *, фильтр_размера: bool = True):
    """Возвращает список (путь, страница, caption).

    Только картинки, у которых есть осмысленный caption — без него LLM
    не может их выбрать, а пользователю они показываются без контекста.
    Если в наборе вообще нет картинок с caption — возвращаем все доступные
    (чтобы не потерять изображения из старых документов, проиндексированных
    до добавления caption-логики).
    """
    if not isinstance(картинки, list):
        return []
    все_доступные: list[tuple] = []
    с_caption: list[tuple] = []
    for картинка in картинки:
        if not isinstance(картинка, dict):
            continue
        путь = _локальный_путь_картинки(картинка.get("path"))
        ...
        запись = (путь, картинка.get("page") or текущая_страница, caption)
        все_доступные.append(запись)
        if caption:
            с_caption.append(запись)
    # Если есть хоть одна с caption — показываем только их;
    # иначе показываем все (fallback для старых документов)
    return с_caption if с_caption else все_доступные


def показать_картинки_фрагмента(фр, key_prefix, *, skip_paths: set[str] | None = None):
    """Показывает картинки фрагмента. skip_paths — пути уже показанных в теле ответа,
    которые не нужно повторять в экспандере фрагмента.

    Правила показа:
    - Сначала собственные картинки страницы, потом соседних страниц.
    - Картинки без caption отображаются только если собственных других нет
      (чтобы не засорять интерфейс логотипами/иконками без описания).
    - Caption отображается под картинкой (не имя файла).
    """
    собственные = _собрать_доступные_картинки_с_caption(фр.get("images"), фр.get("page"))
    if собственные:
        заголовок = "**Изображения со страницы:**"
        список = собственные
    else:
        соседние = _собрать_доступные_картинки_с_caption(
    ...
    # Отфильтровываем картинки, уже показанные в теле ответа
    if skip_paths:
        список = [
            (п, с, cap) for п, с, cap in список
            if str(п.resolve()).lower() not in skip_paths
        ]
    if not список:
        return
    ...
    for индекс, (путь, страница, caption) in enumerate(список):
        with колонки[индекс % len(колонки)]:
            # Показываем осмысленный caption или краткую метку стр./имя файла
            подпись = caption if caption else f"стр. {страница}"
            if len(подпись) > 120:
                подпись = подпись[:117].rstrip() + "…"
            st.image(
                str(путь),
                caption=подпись,
                use_container_width=True,
...
def _разрешить_img_маркер(n: int, m: int, фрагменты: list) -> tuple[Path, str, str] | None:
    ...
    - фрагмент/картинка не существует,
    - файл не найден на диске,
    - картинка слишком маленькая/декоративная.
    """
    if not (1 <= n <= len(фрагменты)):
        return None
    фр = фрагменты[n - 1]
    ...
    if путь is None:
        return None
    # Декоративные/слишком мелкие изображения не показываем в теле ответа
    if not _картинка_содержательная(путь):
        return None
    caption = (карт.get("caption") or "").strip()
    подпись = f"стр. {карт.get('page') or фр.get('page', '')}"
    if caption:
        отображаемый = caption if len(caption) <= 140 else caption[:137].rstrip() + "…"
        подпись = отображаемый
    return путь, подпись, caption


def _валидный_smiles(s: str) -> bool:
    """Минимальная санитарка: строка не пустая, в разумной длине, состоит
    из символов SMILES, не похожа на URL или фразу. Точную проверку делает
    уже smiles-drawer на клиенте."""
    s = (s or "").strip()
    if not s or len(s) > 300 or len(s) < 1:
        return False
    if " " in s or "\n" in s or "\t" in s:
        return False
    # URL-маркеры — точно не SMILES
    if "://" in s or s.lower().startswith(("http", "www.", "ftp", "doi.org")):
        return False
    # SMILES всегда содержит атомы (буквы C/N/O/S/P/H, иногда нижний регистр для ароматики).
    # Проверяем что есть хоть одна разрешённая атомная буква.
    if not re.search(r"[CNOPSFHIBKcnops]", s):
        return False
    if not _SMILES_ВАЛИД.match(s):
        return False
    return True


def _извлечь_smiles_из_ответа(ответ: str) -> tuple[str, list[str]]:
    """Возвращает (текст_с_плейсхолдерами, список_smiles).

    SMILES-блоки заменяются на маркер `<<<SMILES:N>>>`, чтобы потом
    вставить рендер на правильное место в потоке вывода.
    """
    список: list[str] = []

    def _замена(m: re.Match) -> str:
        smi = (m.group(1) or "").strip()
        if not _валидный_smiles(smi):
            return m.group(0)  # оставляем как есть, не наш формат
        список.append(smi)
        return f"\n\n<<<SMILES:{len(список) - 1}>>>\n\n"

    return _SMILES_БЛОК.sub(_замена, ответ), список


_SMILES_МАРКЕР = re.compile(r"<<<SMILES:(\d+)>>>")


def _показать_smiles(smiles_список: list[str], индексы: list[int], *, key_prefix: str) -> None:
    """Рендерит структуры молекул через smiles-drawer (CDN). Несколько
    структур могут идти подряд — выводим в одном блоке.

    smiles-drawer небольшой (~50 KB), грузится с jsDelivr; для оффлайна
    можно положить локально, но для текущего use-case (Streamlit за SSH-
    туннелем) hosted CDN надёжнее всего.
    """
    if not индексы:
        return
    ...
    карточки_html = []
    for поз, idx in enumerate(индексы):
        smi = smiles_список[idx]
        canvas_id = f"smi-{key_prefix}-{idx}-{поз}"
        карточки_html.append(
            f'<div class="smi-card">'
            f'<canvas id="{canvas_id}" width="320" height="220"></canvas>'
            f'<div class="smi-text"><code>{html.escape(smi)}</code></div>'
            f'</div>'
        )
    данные = _json.dumps([
        {"id": f"smi-{key_prefix}-{idx}-{поз}", "smi": smiles_список[idx]}
        for поз, idx in enumerate(индексы)
    ], ensure_ascii=False)
    высота = 280 if len(индексы) <= 3 else 280 * ((len(индексы) + 2) // 3)
    _components.html(
        f"""
<style>
  body {{margin: 0; background: transparent; color: #fafafa; font-family: 'Geist', system-ui, sans-serif;}}
  .smi-grid {{display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 12px;}}
  .smi-card {{background: #111; border: 1px solid #2a2a2a; border-radius: 10px; padding: 12px; text-align: center;}}
  .smi-card canvas {{background: #fff; border-radius: 6px; max-width: 100%; height: auto;}}
  .smi-text {{margin-top: 8px; font-size: 0.78rem; color: #a3a3a3; word-break: break-all;}}
  .smi-text code {{background: transparent; color: #93c5fd;}}
</style>
<div class="smi-grid">{''.join(карточки_html)}</div>
<script src="https://cdn.jsdelivr.net/npm/smiles-drawer@2.1.7/dist/smiles-drawer.min.js"></script>
<script>
(function() {{
  const data = {данные};
  if (!window.SmiDrawer && !window.SmilesDrawer) {{
    // Fallback: библиотека не подгрузилась — показываем только текст
    return;
  }}
  const Drawer = (window.SmilesDrawer && window.SmilesDrawer.Drawer)
    || (window.SmiDrawer && window.SmiDrawer.Drawer);
  const Parser = (window.SmilesDrawer && window.SmilesDrawer.parse)
    || (window.SmiDrawer && window.SmiDrawer.parse);
  if (!Drawer || !Parser) return;
  const drawer = new Drawer({{width: 320, height: 220, padding: 12, themes: {{
    dark: {{
      C: '#111', N: '#3050F8', O: '#FF0D0D', S: '#FFFF30', BACKGROUND: '#fff'
    }}
  }}}});
  data.forEach(({{id, smi}}) => {{
    Parser(smi, function(tree) {{
      ...
      if (el) {{
        ...
        ctx.fillText('Не удалось разобрать SMILES', 10, 30);
      }}
    }});
  }});
}})();
...
def показать_ответ_с_картинками(ответ, фрагменты, key_prefix="rag_answer") -> set[str]:
    ...
    Возвращает множество путей показанных картинок (для подавления дублей в экспандерах).

    Параллельно ловит SMILES-блоки (тройной backtick + 'smiles') — правило
    15 системного промпта — и рендерит молекулы через smiles-drawer.
    """
    if not ответ:
        return set()

    показанные: set[str] = set()  # пути показанных картинок — возвращаемся для подавления дублей

    # C15: сначала вырезаем SMILES в плейсхолдеры, чтобы они не мешали
    # парсингу [img:N.M] и потом стабильно встали на свои места.
    ответ_с_плейсхолдерами, smiles_список = _извлечь_smiles_из_ответа(ответ)
    ...
    def _показать_картинку(n: int, m: int) -> None:
        ...
        if ключ in показанные:
            return
        показанные.add(ключ)
        # Центрируем картинку: ~60% ширины контейнера, чтобы не распирала ответ.
        # При наличии caption показываем его под картинкой.
        _, центр, _ = st.columns([1, 3, 1], gap="small")
        with центр:
            st.image(str(путь), caption=подпись or None, use_container_width=True)

    def _вывести_текст_с_smiles(блок: str) -> None:
        """Выводит markdown-блок, заменяя плейсхолдеры <<<SMILES:N>>> на
        реальный рендер молекулы. Несколько SMILES подряд группируются
        в одну сетку, чтобы не плодить отдельные iframe-ы."""
        if not блок.strip():
            return
        позиция = 0
        очередь: list[int] = []
        for m in _SMILES_МАРКЕР.finditer(блок):
            до = блок[позиция:m.start()]
            # Если до маркера есть осмысленный текст — flush и вывод
            if до.strip():
                if очередь:
                    _показать_smiles(smiles_список, очередь, key_prefix=key_prefix)
                    очередь = []
                st.markdown(вставить_цитаты_в_ответ(до, фрагменты), unsafe_allow_html=True)
            очередь.append(int(m.group(1)))
            позиция = m.end()
        хвост = блок[позиция:]
        if хвост.strip():
            if очередь:
                _показать_smiles(smiles_список, очередь, key_prefix=key_prefix)
                очередь = []
            st.markdown(вставить_цитаты_в_ответ(хвост, фрагменты), unsafe_allow_html=True)
        if очередь:
            _показать_smiles(smiles_список, очередь, key_prefix=key_prefix)
    ...
    позиция = 0
    буфер_текста = ""
    for m in _IMG_МАРКЕР.finditer(ответ_с_плейсхолдерами):
        до_маркера = ответ_с_плейсхолдерами[позиция:m.start()]
        ...
        if текст_для_вывода:
            _вывести_текст_с_smiles(буфер_текста)
        буфер_текста = ""

        _показать_картинку(int(m.group(1)), int(m.group(2)))

    # Хвост ответа после последнего маркера
    хвост = ответ_с_плейсхолдерами[позиция:]
    if хвост.strip():
        _вывести_текст_с_smiles(хвост)
    return показанные


def показать_объяснение_выбранных(результат):
    """UI: чекбоксы у фрагментов уже отрендерены отдельно, здесь — поле
    «уточнить вопрос», кнопка «Объяснить выбранные» и блок результата.

    Когда юзер кликает кнопку — генерируем второй RAG-ответ по тем же
    LLM-настройкам, но передаём в контекст ТОЛЬКО выбранные фрагменты
    (нумерация в ответе пересобирается с 1, чтобы цитаты [N] и маркеры
    картинок [img:N.M] были консистентны с укороченным контекстом).

    Сохраняем результат в `результат['объяснение_выбранных']`. Это
    отдельный блок ниже, не подменяет основной ответ.
    """
    фрагменты = результат.get("фрагменты") or []
    if not фрагменты:
        return
    rag_id = результат.get("id", "rag")
    выбранные_номера = [
        i for i in range(1, len(фрагменты) + 1)
        if st.session_state.get(f"frag_select_{rag_id}_{i}")
    ]

    дизайн.показать_заголовок("Персональное объяснение", отступ_сверху_rem=3)
    st.caption(
        "Отметьте чекбоксы у нужных фрагментов выше — модель ответит, "
        "опираясь только на них. Можно уточнить вопрос или оставить как есть."
    )

    уточнение = st.text_area(
        "Уточнить вопрос (опц.)",
        value="",
        height=70,
        key=f"уточнение_вопроса_{rag_id}",
        placeholder="Например: «Объясни именно эту схему, что обозначают C, T, Ж?»",
    )

    кол = len(выбранные_номера)
    подпись_кнопки = f"Объяснить выбранные ({кол})" if кол else "Объяснить выбранные"
    кнопка_дисабл = (кол == 0)
    if st.button(
        подпись_кнопки,
        key=f"btn_explain_selected_{rag_id}",
        type="primary",
        disabled=кнопка_дисабл,
        use_container_width=True,
    ):
        выбранные_фр = [фрагменты[i - 1] for i in выбранные_номера]
        исходный_вопрос = (результат.get("вопрос") or "").strip()
        полный_вопрос = (
            f"{исходный_вопрос}\n\nДополнительное уточнение пользователя: {уточнение.strip()}"
            if уточнение.strip() else исходный_вопрос
        ) or "Объясни выбранные фрагменты простым языком."
        with st.spinner(f"Генерирую объяснение по {кол} фрагмент(ам)..."):
            ответ_по_выбранным = получить_ответ_от_groq(
                полный_вопрос,
                выбранные_фр,
                llm_id=результат.get("llm_id"),
            )
        результат["объяснение_выбранных"] = {
            "ответ": ответ_по_выбранным,
            "фрагменты": выбранные_фр,
            "номера_исходные": выбранные_номера,
            "уточнение": уточнение.strip(),
        }
        st.session_state["прокрутить_к_ответу"] = False
        st.rerun()

    подсекция = результат.get("объяснение_выбранных")
    if not подсекция:
        return

    st.markdown("---")
    подп_фрагменты = подсекция["фрагменты"]
    подп_ответ = подсекция["ответ"]
    исходные_номера = подсекция.get("номера_исходные") or list(range(1, len(подп_фрагменты) + 1))
    подп_уточнение = подсекция.get("уточнение") or ""

    st.markdown(
        "**Объяснение по выбранным фрагментам** "
        f"({', '.join(f'[{n}]' for n in исходные_номера)}):"
    )
    if подп_уточнение:
        st.caption(f"Уточнение: «{подп_уточнение}»")

    показать_ответ_с_картинками(подп_ответ, подп_фрагменты, key_prefix=f"sel_{rag_id}")
    показать_экспорт_ответа(
        "Объяснение по выбранным фрагментам",
        подп_ответ,
        подп_фрагменты,
        key_prefix=f"sel_{rag_id}_export",
    )
...
def показать_фрагмент_основания(номер, фр, key_prefix, *, select_key: str | None = None, skip_images: set[str] | None = None):
    """Рендерит фрагмент-источник в expander.

    Если задан `select_key` — сверху expander'а добавляется чекбокс «Включить
    в персональное объяснение» с этим ключом. Состояние чекбокса хранится в
    `st.session_state[select_key]`, что позволяет другому коду собрать список
    выбранных фрагментов и попросить LLM объяснить только их.
    """
    ...
    with st.expander(заголовок, expanded=(номер == 1)):
        if select_key:
            st.checkbox(
                "Включить в персональное объяснение",
                key=select_key,
                help=(
                    "Отметьте фрагменты, по которым хотите получить отдельное "
                    "объяснение — кнопка «Объяснить выбранные» появится ниже."
                ),
            )
        метки = []
        if фр.get("domain"):
        ...
        показать_картинки_фрагмента(фр, f"{key_prefix}_{номер}", skip_paths=skip_images)
...
def убрать_неверные_маркеры(ответ, фрагменты, абс_порог=0.65, дельта=0.015, ratio=0.97):
    ...
    try:
        ...
        # Используем предварительно закодированные эмбеддинги фрагментов,
        # если они были сохранены в сериализовать_фрагменты().
        # Это экономит ~30 encode-вызовов на каждый запрос.
        import numpy as np
        if all("_embed" in фр for фр in фрагменты if isinstance(фр, dict)):
            emb_ф = np.array([фр["_embed"] for фр in фрагменты if isinstance(фр, dict)], dtype=np.float32)
        else:
            тексты_ф = [
...
_PDF_МУСОР_ПАТТЕРНЫ = (
    ...
    # C11: служебные заголовки CONTEXT, которые слабые LLM (8B) копируют
    # дословно в ответ: «Источник: корпус. Документ: foo.pdf, стр. 15».
    # Ловим все варианты («мои документы», «корпус», без точки, etc.).
    re.compile(
        r"(?:^|[\s\.\)])"
        r"(?:Источник|Source)\s*:\s*"
        r"(?:корпус|мои\s+документы|user_upload|harvest|corpus)\s*\.?\s*"
        r"(?:Документ|Document)\s*:\s*[^\n]+?,\s*стр\.?\s*\d+\.?",
        re.IGNORECASE,
    ),
    # На случай если модель пишет только «Документ: foo.pdf, стр. 15»
    # как самостоятельную фразу-заголовок.
    re.compile(
        r"(?:^|[\s\.\)])"
        r"(?:Документ|Document)\s*:\s*[^\n,]+\.(?:pdf|docx|txt|pptx)\s*,\s*стр\.?\s*\d+\.?",
        re.IGNORECASE,
    ),
)
...
with вкладка1:
    ...
    if not os.getenv("QDRANT_URL", "").strip():
        ...
        # C3 + C13 fix: при смене локальной базы старый QdrantClient
        # остаётся в @st.cache_resource и держит файловый lock на qdrant_db/.
        # Local mode Qdrant запрещает два клиента на одну папку.
        #
        # Pure cache_resource.clear() недостаточно: он только очищает кеш,
        # но Python-объект QdrantClient ещё живёт в текущем рантайме и
        # держит open file handles + сегменты в RAM (при on_disk + binary
        # quantization это всё равно ~150-300 MB на базу).
        #
        # Чтобы реально освободить RAM при смене 50k → 200k и обратно,
        # 1) явно вызываем .close() у текущего клиента,
        # 2) затем .clear() на cache_resource,
        # 3) gc.collect() — Python иногда держит сегменты qdrant пока
        #    последняя ссылка не GC-нется.
        if выбранная_база != st.session_state.get("qdrant_local_path"):
            try:
                старый_клиент = _загрузить_qdrant_cached(
                    "",
                    None,
                    st.session_state.get("qdrant_local_path") or базы_qdrant[0][1],
                )
                close = getattr(старый_клиент, "close", None)
                if callable(close):
                    close()
            except Exception:
                pass
            try:
                _загрузить_qdrant_cached.clear()
            except Exception:
                pass
            try:
                import gc
    ...
    к1, к2 = st.columns([1.7, 1], gap="small")
    with к1:
    ...
    доступные_llm = list(LLM_МОДЕЛИ.keys())
    модель_по_умолчанию = st.session_state.get("llm_id") or _llm_по_умолчанию()
    индекс_llm = доступные_llm.index(модель_по_умолчанию) if модель_по_умолчанию in доступные_llm else 0
    выбранная_llm = st.selectbox(
        "Модель ответа",
        options=доступные_llm,
        index=индекс_llm,
        format_func=_llm_label,
        help="DeepSeek использует DEEPSEEK_API_KEY из .env; Groq использует GROQ_API_KEY.",
    )
    st.session_state["llm_id"] = выбранная_llm

    # C16: расширенный режим — два прохода LLM.
    #   Проход 1: основная модель составляет первичный ответ по фрагментам.
    #   Проход 2: вторая модель (или та же) видит и фрагменты, и первичный
    #             ответ, и добавляет факты которые были упущены.
    # Стоит ~2x токенов, поэтому опция, не дефолт. Полезно когда нужен
    # максимально полный пересказ корпуса (учебная задача, обзор).
    глубокий_режим = st.checkbox(
        "Углублённый ответ (двойной проход LLM, точнее но медленнее)",
        value=False,
        key="углублённый_режим",
        help=(
            "Первый проход — основной ответ по фрагментам. "
            "Второй проход — модель видит свой первый ответ и фрагменты, "
            "добавляет недостающие факты, формулы, методы, классы соединений. "
            "Если выбрана LLaMA, второй проход делает её же; если есть "
            "DeepSeek и LLaMA — используется кросс-проверка."
        ),
    )

    # Кнопка действия — отдельной строкой, прижата к правому краю.
    # Раньше она делила ряд со слайдером и потребовался невидимый
    # vertical-spacer для align по нижнему краю — это читалось как «костыль».
    _, действие = st.columns([3, 1], gap="small")
    with действие:
    ...
    if кнопка and вопрос_пользователя.strip():
        try:
            ...
            if режим_запроса == "my":
                ...
                точки = мои_точки + корпус_точки
            else:
                точки = корпус_точки

            точки = _диверсифицировать_документы(точки, количество_фрагментов)
            ...
            if not точки:
                ...
                углуб_payload = None
                if глубокий_режим:
                    with st.spinner("Углублённый ответ: проход 1 → проход 2 (~2x времени)..."):
                        результат_углуб = получить_углублённый_ответ(
                            вопрос_пользователя, точки, llm_id=выбранная_llm,
                        )
                    ответ = результат_углуб["финал"]
                    # Если второй проход реально что-то изменил (черновик ≠ финал) —
                    # сохраняем оба, UI покажет рассуждения обеих моделей.
                    если_черновик_отличается = (
                        результат_углуб.get("черновик")
                        and результат_углуб["черновик"].strip() != ответ.strip()
                    )
                    if если_черновик_отличается:
                        углуб_payload = {
                            "черновик": результат_углуб["черновик"],
                            "llm1": результат_углуб.get("llm1"),
                            "llm2": результат_углуб.get("llm2"),
                        }
                else:
                    with st.spinner(f"Генерация ответа · {_llm_label(выбранная_llm).replace(' (нет ключа)', '')}..."):
                        ответ = получить_ответ_от_groq(вопрос_пользователя, точки, llm_id=выбранная_llm)
                st.session_state.результаты_поиска = {
                    "тип": "rag",
                    ...
                    "заметка": заметка,
                    "ответ": ответ,
                    "llm_id": выбранная_llm,
                    "углублённый": углуб_payload,
                    "фрагменты": сериализовать_фрагменты(точки),
            ...
            # Дублируем в stderr — попадёт в _streamlit.log при запуске через scripts/запуск.py.
            print(трейс, file=sys.stderr, flush=True)
            st.session_state.результаты_поиска = None
            st.error(f"Ошибка поиска: {type(ошибка).__name__}: {ошибка}")
            with st.expander("Подробности (traceback)", expanded=False):
                st.code(трейс, language="python")
    elif кнопка:
    ...
    if результат and результат.get("тип") == "off_topic":
        ...
        # Показываем реальную выбранную модель, а не хардкод llama-70b.
        # Если ответ — углублённый, показываем обе модели.
        meta_данные_llm = LLM_МОДЕЛИ.get(результат.get("llm_id") or "") or {}
        meta_label = meta_данные_llm.get("label") or "LLM"
        meta_label_short = meta_label.replace("Groq · ", "").replace("DeepSeek · ", "DeepSeek ")
        углуб_данные = результат.get("углублённый") or {}
        if углуб_данные:
            проход1_label = LLM_МОДЕЛИ.get(углуб_данные.get("llm1") or "", {}).get("label") or "LLM 1"
            проход2_label = LLM_МОДЕЛИ.get(углуб_данные.get("llm2") or "", {}).get("label") or "LLM 2"
            кросс = (
                проход1_label.replace("Groq · ", "").replace("DeepSeek · ", "DeepSeek ")
                + " → "
                + проход2_label.replace("Groq · ", "").replace("DeepSeek · ", "DeepSeek ")
            )
            дизайн.показать_мета_rag(len(фрагменты), модель=кросс)
        else:
            дизайн.показать_мета_rag(len(фрагменты), модель=meta_label_short)
        ...
        уже_показанные_пути = показать_ответ_с_картинками(ответ, фрагменты)

        # Если был углублённый режим и черновик реально отличается от финала —
        # показываем рассуждения обеих моделей в expander'е.
        если_углублённый = результат.get("углублённый")
        if если_углублённый:
            llm1_label = LLM_МОДЕЛИ.get(если_углублённый.get("llm1") or "", {}).get("label") or "Проход 1"
            llm2_label = LLM_МОДЕЛИ.get(если_углублённый.get("llm2") or "", {}).get("label") or "Проход 2"
            with st.expander(f"Рассуждения двух моделей · {llm1_label} → {llm2_label}", expanded=False):
                st.markdown(f"**Первый проход — {_html(llm1_label)}** (черновик):", unsafe_allow_html=True)
                st.markdown(
                    вставить_цитаты_в_ответ(если_углублённый["черновик"], фрагменты),
                    unsafe_allow_html=True,
                )
                st.markdown("---")
                st.markdown(
                    f"**Второй проход — {_html(llm2_label)}** (финал, показан выше). "
                    "Этот проход видел и фрагменты, и черновик первой модели, "
                    "и дополнял недостающие факты.",
                    unsafe_allow_html=True,
                )
        ...
        rag_id = результат.get("id", "rag")
        select_key_for = lambda i: f"frag_select_{rag_id}_{i}"  # noqa: E731
        ...
        if номера_цитат:
            ...
            for номер in номера_цитат:
                показать_фрагмент_основания(
                    номер, фрагменты[номер - 1], "used_fragment",
                    select_key=select_key_for(номер),
                    skip_images=уже_показанные_пути,
                )
            ...
            if остальные:
                ...
                for номер in остальные:
                    показать_фрагмент_основания(
                        номер, фрагменты[номер - 1], "extra_fragment",
                        select_key=select_key_for(номер),
                        skip_images=уже_показанные_пути,
                    )
        else:
            ...
            for i, фр in enumerate(фрагменты, 1):
                показать_фрагмент_основания(
                    i, фр, "diagnostic_fragment",
                    select_key=select_key_for(i),
                )

        показать_объяснение_выбранных(результат)
        ...
        if not файлы:
            ...
            for файл in файлы:
                with st.expander(f"{файл['name']} · {файл.get('chunks', 0)} чанков"):
                    ...
                    if файл.get("reindexed_at"):
                        st.caption(f"Переиндексирован: {файл['reindexed_at']}")
                    if файл.get("use_ocr"):
                        st.caption("OCR картинок: ✓ применён")
                    тип_файла = (файл.get("type") or "").lower().lstrip(".")
                    if тип_файла == "pdf":
                        ключ_кнопки = f"reindex_ocr_{файл.get('file_hash', '')}"
                        if st.button(
                            "Переиндексировать с OCR",
                            key=ключ_кнопки,
                            help="Заново обработать этот PDF с распознаванием текста на схемах "
                                 "(easyocr). Старые точки в Qdrant удалятся, придут новые "
                                 "с обогащёнными captions картинок. Сам PDF и сохранённые "
                                 "картинки не трогаются.",
                        ):
                            прогресс = st.empty()
                            def _on_reindex_progress(текст: str, _плейсхолдер=прогресс) -> None:
                                _плейсхолдер.caption(текст)
                            try:
                                with st.spinner("Переиндексация с OCR (~1 мин на 100 картинок)…"):
                                    итог_реиндекс = notebooks.переиндексировать_файл_с_ocr(
                                        загрузить_qdrant(),
                                        загрузить_модель(),
                                        выбранная_для_документов["id"],
                                        файл.get("file_hash", ""),
                                        user_id=пользователь_id,
                                        on_progress=_on_reindex_progress,
                                    )
                                прогресс.empty()
                                st.success(
                                    f"Готово: {итог_реиндекс['chunks']} чанков на "
                                    f"{итог_реиндекс['pages']} страницах. "
                                    "Теперь поиск находит слова с картинок-схем."
                                )
                                st.rerun()
                            except Exception as ошибка_реиндекс:
                                прогресс.empty()
                                st.error(f"Ошибка переиндексации: {ошибка_реиндекс}")
