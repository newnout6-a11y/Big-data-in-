...
def _диверсифицировать_документы(
    ...
    *,
    ...
    """Мягкий MMR-lite по документам.

    Сначала берём максимум `max_per_document` фрагмента из одного документа,
    но только среди достаточно релевантных кандидатов. Если альтернатив мало,
    добираем лучшими фрагментами без ограничения — так не теряем ответ, когда
    он действительно есть только в одном PDF.
    """
    ...
    for т in кандидаты:
        if len(выбранные) >= количество:
            break
        if порог is not None and float(getattr(т, "score", 0.0) or 0.0) < порог:
            continue
        док = _ключ_документа(т)
        if по_документам.get(док, 0) >= max_per_document:
            continue
        выбранные.append(т)
...
def получить_ответ_от_groq(вопрос, фрагменты, *, llm_id=None):
    ...
    if данные_llm.get("provider") == "deepseek":
        доп_инструкция = (
            "\n\nВАЖНО ДЛЯ DEEPSEEK: CONTEXT уже найден поисковой системой. "
            "Не отвечай 'В базе нет данных' только из-за недословного совпадения вопроса. "
            "Сначала извлеки любые частичные факты из фрагментов [1]...[N]. "
            "Отказ допустим только если во всех фрагментах нет ни одного содержательного факта по теме вопроса."
        )
...
with вкладка1:
    ...
    if not os.getenv("QDRANT_URL", "").strip():
        ...
        # C3 fix: при смене локальной базы старый QdrantClient остаётся в
        # @st.cache_resource и держит файловый lock на qdrant_db/. Local mode
        # Qdrant запрещает два клиента на одну папку — повторный выбор той же
        # базы может бросить «already accessed by another instance». Плюс
        # копятся клиенты в памяти. Чистим кэш при изменении пути.
