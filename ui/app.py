...
5. Если CONTEXT содержательно не относится к вопросу (говорит о принципиально другой теме без единой формулы или факта по вопросу) — скажи: «В базе нет данных для ответа на этот вопрос» — и на этом остановись, больше ничего не добавляй. ВАЖНО: прежде чем дать такой отказ, перечитай КАЖДЫЙ фрагмент по отдельности — если хотя бы в одном есть относящееся к вопросу утверждение, определение, число, метод или формула, ты ОБЯЗАН ответить по нему и НЕ отказывать. Наличие во фрагментах ссылок, DOI, имён авторов или иных PDF-артефактов (см. правило 12) само по себе НЕ является основанием для отказа, если рядом есть содержательный текст.
...
def похоже_на_мусор(текст):
    ...
    # Формульные/числонасыщенные фрагменты НЕ считаем мусором по доле цифр и
    # букв: уравнения, индексы и числовые константы дают высокую долю цифр и
    # низкую долю «словесных» букв, но это ровно тот контент, который требуют
    # правила 9/10 промпта. Без этой защиты фрагмент с ключевой формулой
    # выбрасывался ещё до LLM, и модель отвечала «в базе нет данных».
    есть_математика = содержит_математику(текст)

    числовых = sum(1 for с in т if с.isdigit() or с in " .,()/-+|")
    if not есть_математика and числовых / len(т) > 0.45:
        return True


    слова_букв = re.findall(r"[а-яёa-z]{3,}", т)
    длина_букв = sum(len(с) for с in слова_букв)
    if not есть_математика and длина_букв / len(т) < 0.40:
        return True
    return False
...
def _диверсифицировать_по_документам(кандидаты, количество, *, макс_на_документ=2):
    """Спред top-k по разным документам с сохранением порядка релевантности.

    Жадно идём по кандидатам в исходном (релевантном) порядке и берём не более
    `макс_на_документ` фрагментов из одного файла, пока не наберём `количество`.
    Если разных документов не хватило — добираем оставшиеся самые релевантные
    фрагменты без лимита. Лечит «10 цитат из одного документа»: ответ и блок
    «Источники» получают несколько разных источников, когда они есть в выдаче.
    """
    if количество <= 0 or len(кандидаты) <= 1:
        return кандидаты
    счётчик = {}
    отобранные = []
    остаток = []
    for т in кандидаты:
        payload = т if isinstance(т, dict) else (getattr(т, "payload", None) or {})
        док = str(payload.get("document", ""))
        if len(отобранные) < количество and счётчик.get(док, 0) < макс_на_документ:
            отобранные.append(т)
            счётчик[док] = счётчик.get(док, 0) + 1
        else:
            остаток.append(т)
    if len(отобранные) < количество:
        отобранные.extend(остаток[: количество - len(отобранные)])
    return отобранные
...
def найти_похожие(
    ...
    *,
    ...
    # Берём широкий пул кандидатов (≥30), чтобы было из чего диверсифицировать
    # выдачу по разным документам ниже. Без этого top-k часто оказывался весь
    # из одного файла → в «Источниках» один документ с десятком цитат.
    лимит_кандидатов = min(max(количество * 5, 30), 60)
    ...
    # При гибриде у точек RRF-score (~0.01–0.1) — порог по-другому. При dense — cosine-порог ниже (см. ниже).
    if гибрид:
        кандидаты = list(ответ.points)
    else:
        # Порог снижен 0.72 → 0.6: для кросс-языковых запросов (RU-вопрос ↔
        # EN-пассаж) у multilingual-e5 косинус релевантной пары часто 0.6–0.72,
        # и старый порог отбрасывал верные фрагменты ещё до фильтров.
        МИН_СХОДСТВО = float(os.getenv("MIN_DENSE_SIMILARITY", "0.6"))
    ...
    if использовать_reranker and кандидаты:
        try:
            ...
            # top_k=None: переранжируем ВЕСЬ пул и сохраняем полный порядок,
            # чтобы диверсификация ниже могла выбрать разные документы. Раньше
            # reranker обрезал до `количество` ДО диверсификации — и если топ был
            # из одного файла, спред было уже не из чего делать.
            упорядоченные = переранжировать(вопрос, тексты)
    ...
    кандидаты = _диверсифицировать_по_документам(кандидаты, количество)
...
def показать_объяснение_выбранных(результат):
    ...
    if st.button(
        ...
        with st.spinner(f"Генерирую объяснение по {кол} фрагмент(ам)..."):
            ответ_по_выбранным = получить_ответ_от_groq(полный_вопрос, выбранные_фр)
        ...
        try:
            ...
            if not точки:
                ...
                st.session_state.результаты_поиска = {
                    ...
                    "ответ": ответ,
                    "фрагменты": сериализовать_фрагменты(точки),
                    # Сохраняем исходный вопрос и уникальный id поиска — оба
                    # нужны для фичи «Объяснить выбранные фрагменты»: id
                    # привязывает чекбоксы к этому конкретному поиску, чтобы
                    # они не путались при следующем запросе.
