...
15. SMILES-СТРОКИ — структуры молекул. Если в CONTEXT встречается SMILES-нотация конкретного соединения (например `CC(=O)Oc1ccccc1C(=O)O` для аспирина) И эта молекула релевантна вопросу, ОБЯЗАТЕЛЬНО вынеси её в ответ в блоке кода с ярлыком `smiles`:

```smiles
CC(=O)Oc1ccccc1C(=O)O
```

UI распознаёт такие блоки и рисует структурную формулу. Можно ставить несколько SMILES-блоков для разных молекул. Маркер [N] ставь сразу после блока. Не выдумывай SMILES — только из CONTEXT. Не путай SMILES с обычной формулой Hill (`C9H8O4` — это формула, а не SMILES; для неё блок smiles НЕ нужен)."""
...
def получить_углублённый_ответ(вопрос, фрагменты, *, llm_id=None):
    ...
    промпт_расширения = (
        ...
        " — пропущенные методы, формулы, химические реакции и SMILES из CONTEXT;\n"
            ...
            if not точки:
                ...
                if глубокий_режим:
                    with st.spinner("Углублённый ответ: проход 1 → проход 2 (~2x времени)..."):
                        ответ = получить_углублённый_ответ(
