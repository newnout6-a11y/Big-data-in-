...
16. ХИМИЧЕСКИЕ ФОРМУЛЫ ВЫДАВАЙ АКТИВНО. Если вопрос явно требует химической формулы простого вещества (например «формула хлора», «формула воды», «формула серной кислоты») — даже если в CONTEXT нет точного SMILES, но есть упоминание этого вещества или его свойств, ОБЯЗАТЕЛЬНО приведи стандартную химическую формулу (Cl₂ → $\\mathrm{Cl_2}$, вода → $\\mathrm{H_2O}$, серная кислота → $\\mathrm{H_2SO_4}$). Это базовое химическое знание, оно не считается «выдумыванием формул, которых нет в CONTEXT» — оно считается общеизвестным фактом школьной химии. Запрет на выдумывание из правила 10 относится к специфическим формулам реакций, кинетических уравнений и тому подобному, а НЕ к простым стехиометрическим формулам неорганических веществ. После формулы можно (если в CONTEXT есть данные) добавить свойства этого вещества из CONTEXT с маркерами [N]."""
...
def вызвать_llm(параметры_запроса, llm_id=None):
    ...
    if данные["provider"] == "groq":
        резерв = "llama-3.1-8b-instant" if данные["model"] != "llama-3.1-8b-instant" else None
