...
def получить_ответ_от_groq(вопрос, фрагменты, *, llm_id=None):
    ...
    # C22: fallback на общие знания. Если retrieval вернул фрагменты, но
    # они явно не про тему вопроса (score низкий или ключевые слова вопроса
    # не встречаются ни в одном фрагменте), модель всё равно отказывает
    # «В базе нет данных». Это правильно по правилу 5 — но пользователь
    # ожидает хоть какой-то ответ на базовый химический вопрос.
    #
    # Решение: добавляем MODE: fallback_general_knowledge в user-message.
    # Правило 16 в системном промпте разрешает модели ответить из общих
    # знаний, честно пометив это курсивом. Никакого хардкода формул.
    #
    # Критерий «фрагменты нерелевантны»: ни одно слово из вопроса (≥4 букв)
    # не встречается ни в одном фрагменте. Это грубый, но быстрый фильтр.
    def _фрагменты_релевантны(вопрос_текст: str, фрагменты_список: list) -> bool:
        слова_вопроса = {
            w.lower() for w in re.findall(r"[\wёЁ]+", вопрос_текст)
            if len(w) >= 4
        }
        if not слова_вопроса:
            return True  # слишком короткий вопрос — не можем судить
        все_тексты = " ".join(
            (фр.get("text") if isinstance(фр, dict) else фр.payload.get("text", ""))
            for фр in фрагменты_список
        ).lower()
        совпало = sum(1 for w in слова_вопроса if w in все_тексты)
        # Если хотя бы 30% слов вопроса нашлись — считаем релевантным
        return совпало / max(len(слова_вопроса), 1) >= 0.3

    fallback_mode = not _фрагменты_релевантны(вопрос, фрагменты)
    mode_tag = "\n\nMODE: fallback_general_knowledge" if fallback_mode else ""
