...
_СХЕМА_СТЕМЫ = frozenset({"схем", "диагр", "рисун", "черте", "граф", "изобр", "слайд", "слайд"})
_IMAGE_БУСТ = 0.12
...
def _keyword_буст(payload: dict[str, Any], стемы_запроса: set[str]) -> float:
    ...
    image_score = _IMAGE_БУСТ if (стемы_запроса & _СХЕМА_СТЕМЫ and payload.get("images")) else 0.0
...
def _заголовок_слайда(текст: str) -> str:
    """Возвращает «суть» слайда — конкатенацию заголовка и тела до ~400 симв.

    Эвристика для PDF-слайдов лекций: подписей `Fig. N` / `Рис. N` нет,
    но содержание самого слайда обычно описывает картинку. Заголовок
    слайда часто общий («Классификация стадий ХТП»), а ключевые слова
    («гидромеханические процессы», «фильтрование») — в теле. Поэтому
    в caption включаем и заголовок, и тело, обрезая по символам, а не
    по первой точке/двоеточию.
    """
    if not текст:
        return ""
    ...
    собрано = 0
    for s in строки:
        # +1 за пробел между кусками
        if собрано and собрано + 1 + len(s) > _МАКС_CAPTION_СЛАЙДА:
            # Если ещё ничего не накопили, всё равно положим первую строку
            if not куски:
                куски.append(s)
            break
        куски.append(s)
