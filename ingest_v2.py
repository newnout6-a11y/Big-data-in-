...
def _извлечь_картинки_страницы(документ, страница, номер_страницы, папка_документа):
    # В harvester pipeline картинки не нужны — они только раздувают Drive.
    # Полное извлечение оставлено для notebooks (UI), там пользователь
    # явно прогоняет ingest через notebooks.py с save_images=True.
