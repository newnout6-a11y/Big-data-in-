...
7. ОБЯЗАТЕЛЬНО ставь маркер [N] сразу после каждого утверждения в теле ответа, где N — номер фрагмента из CONTEXT, из которого взят факт. Один маркер — на одно предложение или короткий абзац. Можно ставить несколько маркеров [1][3], если факт подтверждается двумя фрагментами. Без маркеров ответ считается неполным.
...
9. Все математические формулы оборачивай в двойные доллары для блочного вида: $$...$$ или одинарные для inline: $...$. Используй ТОЛЬКО стандартный LaTeX-синтаксис: \\sigma вместо σ, \\sum_{i=1}^{n} вместо ∑, \\in вместо ∈, \\mathcal{N}(v) для множеств, h^{(t+1)}_v для индексов и степеней, \\cdot вместо *. Не используй Unicode-математику и псевдо-LaTeX типа h_v^(t+1). Каждая формула — отдельный блок $$...$$ на своей строке с пустыми строками до и после."""
...
def вставить_цитаты_в_ответ(текст, фрагменты):
    ...
    def замена(совпадение):
        ...
        цитата = полный_текст[:350]
        if len(полный_текст) > 350:
            цитата += "…"
