...
_МУСОРНЫЕ_МАРКЕРЫ = (
    ...
    # Колонтитулы и masthead научных журналов (рус и англ)
    ...
    # Конфликт интересов и финансирование (служебные разделы)
    "конфликт интересов",
    "конфликта интересов",
...
_PDF_ЗАМЕНЫ = (
    ("¼", "="),  # cmap-каша: знак равенства в формулах PDF превращается в дробь 1/4
    ("þ", "+"),  # исландский thorn → плюс
    ("ð", "("),  # исландский eth (строчная) → открывающая скобка
    ("Þ", ")"),  # исландский Thorn (заглавная) → закрывающая скобка
    ("¦", "|"),  # сломанная вертикальная черта
    ("\u00ad", ""),  # мягкий перенос — мешает поиску и читаемости
    ("\ufb01", "fi"),  # лигатура fi
    ("\ufb02", "fl"),  # лигатура fl
)
...
def почистить_pdf_артефакты(текст):
    """Чистит типичные cmap-артефакты PDF — ¼ вместо =, þ вместо + и т.п.
    Применяется при чтении из Qdrant перед показом тултипа и передачей в LLM."""
    if not текст:
        return текст
    для_замены = текст
...
def похоже_на_мусор(текст):
    ...
    if any(маркер in т for маркер in _МУСОРНЫЕ_МАРКЕРЫ):
        return True
    # Поток цифр и разделителей: >45% — это таблицы или последовательности кодов
    числовых = sum(1 for с in т if с.isdigit() or с in " .,()/-+|")
    if числовых / len(т) > 0.45:
        return True
    # Доля «связных слов» (3+ строчных букв подряд) — должно быть ≥40% от длины.
    # Иначе это коды генов (MW535279, AMP/CIP/S/CTX), формулы или подписи к таблицам.
...
def _вынести_маркеры_из_формул(текст):
    """Переносит маркеры [N] за пределы $...$ и $$...$$ блоков LaTeX.

    KaTeX парсит содержимое между долларами как математику и съедает любую
    HTML-разметку внутри. Если LLM поставила [2] внутри $$...$$, наш span с
    тултипом исчезает — маркер становится некликабельным сырым текстом.
    Чтобы этого избежать, до замены [N] на <span> вытаскиваем все маркеры
    из формульных блоков и приклеиваем сразу после блока — там они
    нормально превратятся в кликабельную цитату."""
    def обработать_блок(m):
        блок = m.group(0)
        маркеры = re.findall(r"\[\d+\]", блок)
        if not маркеры:
            return блок
        # Удаляем сами маркеры, но не трогаем окружающие пробелы и переносы:
        # внутри формулы они нужны для корректного KaTeX-рендера (\cdot, \frac).
        чистый = re.sub(r"\[\d+\]", "", блок)
        return чистый + "".join(маркеры)

    # Сначала блочные $$...$$ (могут содержать переносы строк), потом inline $...$
...
def вставить_цитаты_в_ответ(текст, фрагменты):
    """Заменяет [N] в ответе LLM на span с тултипом, содержащим текст фрагмента N.
    На наведение пользователь видит реальную цитату — это доказывает что
    модель не выдумала источник."""
    if not фрагменты:
        return текст
    # Если LLM не послушалась правила 8 и добавила свой раздел «Источники:»
    # — обрезаем его, чтобы не дублировать наш собственный блок ниже.
    очищенный = re.sub(
    ...
    # Маркеры внутри $$...$$ / $...$ KaTeX съедает — переносим их наружу
    # ДО экранирования и замены на span, чтобы все цитаты остались кликабельными.
    очищенный = _вынести_маркеры_из_формул(очищенный)
    # Экранируем HTML в самом ответе, чтобы случайные < > & не сломали разметку.
    # Но markdown (** _ # $ $$) после этого продолжит работать.
    безопасный = (
        очищенный.replace("&", "&amp;")
                 .replace("<", "&lt;")
                 .replace(">", "&gt;")
...
def найти_похожие(вопрос, выбранный_кейс, количество):
    ...
    # Чистим PDF-артефакты в payload каждой точки: и тултип, и LLM получат
    # текст без ¼/þ/ð/Þ-каши, формулы в цитатах будут читаемые.
    for точка in ответ.points:
        точка.payload["text"] = почистить_pdf_артефакты(точка.payload.get("text", ""))

    # Порог релевантности: multilingual-e5 для совсем нерелевантных вопросов
    # даёт скоры < 0.72, для пограничных 0.72-0.80, для релевантных ≥ 0.80.
    # 0.72 отсекает явный шум, но пропускает пограничные ответы — пусть LLM
    # сама решит на основе содержания, релевантно или нет.
    МИН_СХОДСТВО = 0.72
...
def убрать_неверные_маркеры(ответ, фрагменты, абс_порог=0.65, дельта=0.03):
    """Пост-проверка каждого маркера [N] в ответе через embedding-сравнение.
    Для каждого маркера берём предшествующее утверждение и считаем косинусное
    сходство с текстом фрагмента N. Маркер считаем ошибочным если:
      - sim < абс_порог (фрагмент N вообще не про это утверждение), ИЛИ
      - sim < max_sim - дельта (есть фрагмент существенно ближе к утверждению).

    Семантически близкие фрагменты (об одной теме) дают cos-sim в пределах
    0.05-0.08 друг от друга, поэтому дельта 0.03 — баланс: ловит явные
    промахи LLM (где правильный фрагмент отстаёт ≥ 0.04), но не трогает
    случаи когда несколько фрагментов реально подтверждают утверждение.

    Это ловит случаи когда LLM ставит [N] на утверждение, которого в
    фрагменте N нет (перепутала номер). Чистый промпт-контроль с этим
    не справляется."""
    ...
    for m in re.finditer(r"\[(\d+)\]", ответ):
        ...
        if not (1 <= n <= len(фрагменты)):
            continue
        # Утверждение = текст с ближайшей границы (точка/вопрос/перенос/запятая) до маркера
        start = m.start()
        границы = [
        ...
        # Убираем сами маркеры [N] из текста утверждения — они не должны влиять
        # на embedding (это наша служебная разметка, а не содержание)
        утв = re.sub(r"\[\d+\]", "", утв_сырой).strip()
        # Короткие фразы-связки не проверяем — там нечего сравнивать
        if len(утв) < 20:
            continue
    ...
    try:
        ...
        матрица = emb_утв @ emb_ф.T  # [len(уник_утв), len(фрагменты)]
        ...
        for м in маркеры:
            ...
            if sim < абс_порог or sim < max_sim - дельта:
                для_удаления.append((м["start"], м["end"]))
    except Exception:
        # При ошибке модели/numpy — лучше вернуть как есть, чем упасть
        return ответ

    if not для_удаления:
        return ответ

    # Удаляем с конца, чтобы не сдвигались индексы; убираем пробел перед маркером
    ...
    # Схлопываем случайные двойные пробелы, оставшиеся после удалений
...
with вкладка1:
    ...
    if результат and результат["тип"] == "демо":
        ...
        # Источники показываем тогда и только тогда, когда LLM реально
        # использовала маркеры [N] в ответе (хоть в основном тексте, хоть в
        # разделе «Косвенно по теме»). Если маркеров нет — модель отказала
        # без привязки к фрагментам, и блок Источники станет обманчивым.
