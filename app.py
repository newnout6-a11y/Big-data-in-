...
5а. ВАЖНО: даже при отказе по правилам 4 или 5, ПОСЛЕ основного отказа ОБЯЗАТЕЛЬНО добавь на новой строке раздел «**Косвенно по теме в найденных фрагментах:**» и перечисли 1–3 пункта в формате: «во фрагменте [N] обсуждается тема X» — указывая конкретные темы и предметы, которые реально присутствуют в CONTEXT, с маркерами [N]. Это поможет пользователю понять, какая близкая информация есть в базе, и переформулировать вопрос. Не выдумывай — пиши только то, что реально упомянуто в CONTEXT. Если все фрагменты — это титулки журналов, метаданные или конфликт интересов, не имеющие содержательной информации — раздел «Косвенно по теме» можно опустить.
