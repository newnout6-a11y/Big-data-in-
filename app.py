...
def убрать_неверные_маркеры(ответ, фрагменты, абс_порог=0.65, дельта=0.015, ratio=0.97):
    ...
    сходство с текстом фрагмента N. Маркер считаем ошибочным если:
      ...
      - sim < max_sim - дельта (есть фрагмент заметно ближе к утверждению), ИЛИ
      - sim < max_sim * ratio (sim хуже max более чем на 3% относительно).

    Двойной критерий (абсолютный + относительный) ловит пограничные случаи:
    например при max=0.84 удалится всё что < max-0.015=0.825 и < max*0.97=0.815.
    Маркеры в самой "верхушке" (sim ≈ max в пределах 1.5%) сохраняются —
    это случаи реальной множественной поддержки.
