...
with вкладка1:
    ...
    if результат and результат["тип"] == "демо":
        ...
        # Детектим явный отказ LLM («В базе нет данных…», «преимущественно
        # библиографические…»). При отказе блок Источники не имеет смысла —
        # маркеры [N] в ответе не использованы, ничего не подтверждается.
        # Найденные фрагменты оставляем в expander ниже — для прозрачности,
        # пользователь увидит почему модель отказала.
        отказ_фразы = (
            "в базе нет данных",
            "преимущественно библиографические",
            "найденных фрагментах преимущественно",
            "не относится к вопросу",
        )
        это_отказ = any(ф in ответ.lower() for ф in отказ_фразы)
        ...
        if это_отказ:
            st.caption(
                ...
                "но модель не нашла в них прямого ответа. Поэтому раздел "
                "«Источники» не показан."
            )
